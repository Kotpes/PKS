...
<?php
		...
		if($login->display_name == ''){ 
			...
			$registerFilterArray['loginLogoutError'] = '
				<p class="log-in-out alert">'. __('Olet kirjautunut sisään nimellä', 'profilebuilder') .' <a href="'.get_author_posts_url( $login->ID ).'" title="'.$login->display_name.'">'.$login->display_name.'</a>. '. __('Et tarvitse toista tiliä.', 'profilebuilder') .' <a href="'.wp_logout_url(get_permalink()).'" title="'. __('Log out of this account.', 'profilebuilder') .'">'. __('Kirjaudu ulos', 'profilebuilder') .'  &raquo;</a></p><!-- .log-in-out .alert -->';
								...
								if ($wppb_defaultOptions['email'] == 'show'){
									...
									$registerFilterArray2['name2'] = '
										<p class="form-email'.$errorVar.'">
											<label for="email">'. __('Email', 'profilebuilder') .$errorMark.'</label><br />
						...
						if ($wppb_defaultOptions['firstname'] == 'show'){
							...
							$registerFilterArray2['name3'] = '
								<p class="first_name'.$errorVar.'">
									<label for="first_name">'. __('Etunimi', 'profilebuilder') .$errorMark.'</label><br />
						...
						if ($wppb_defaultOptions['lastname'] == 'show'){ 
							...
							$registerFilterArray2['name4'] = '
								<p class="last_name'.$errorVar.'">
									<label for="last_name">'. __('Sukunimi', 'profilebuilder') .$errorMark.'</label><br />
						...
						if ($wppb_defaultOptions['password'] == 'show'){
							...
							$registerFilterArray2['ay3'] = '
								<p class="form-password'.$errorVar.'">
									<label for="pass1">'. __('Salasana', 'profilebuilder') .$errorMark.'</label><br />
									<input class="text-input" name="passw1" type="password" id="pass1" value="'.trim($localVar1).'" />
								</p><!-- .form-password -->
				 
								<p class="form-password'.$errorVar2.'">
									<label for="pass2">'. __('Salasana uudestan', 'profilebuilder') .$errorMark2.'</label> <br >
...
?>
						...
						<p class="form-submit">
							...
							?>
							<input name="adduser" type="submit" id="addusersub" class="submit button" value="REKISTERÖIDY" />
