...
function wppb_front_end_login( $atts ){
	...
	if ( is_user_logged_in() ){  // Already logged in 
		...
		$loginFilterArray['loginMessage1'] = '<p class="alert">'. sprintf(__('Olet tällä hetkellä kirjautunut sisään osoitteella %1$s. %2$s', 'profilebuilder'), '<a href="'.$authorPostsUrl = get_author_posts_url( $wppb_user->ID ).'" title="'.$display_name.'">'.$display_name.'</a>', '<a href="'.wp_logout_url( $redirectTo = wppb_curpageurl() ).'" title="'. __('Log out of this account', 'profilebuilder').'">'. __('Kirjaudu ulos', 'profilebuilder').' &raquo;</a>') . '</p><!-- .alert-->';
		...
		<?php
			...
			$loginFilterArray['loginUsername'] = '
				<p class="login-form-username">
					<label for="user-name">'. $loginWith .'</label><br />
			...
			$loginFilterArray['loginPassword'] = '
				<p class="login-form-password">
					<label for="password">'. __('Salasana', 'profilebuilder') .'</label><br />
		...
		?>
			<p class="login-form-submit">
				...
				<input type="submit" name="submit" class="submit button" value="kirjaudu" />
				<?php
					...
					<p class="remember-me-container">
						...
						<label for="remember-me">'. __('Muista minut', 'profilebuilder').'</label></p>';
